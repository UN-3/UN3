...
import org.un.common.crypto.jce.UnCastleProvider;
...
public class ECKey implements Serializable, SignInterface {
  ...
  public ECKey(SecureRandom secureRandom) {
    this(UnCastleProvider.getInstance(), secureRandom);
  }
  ...
  public ECKey(byte[] key, boolean isPrivateKey) {
    ...
    this.provider = UnCastleProvider.getInstance();
  }
  ...
  private static PrivateKey privateKeyFromBigInteger(BigInteger priv) {
    if (priv == null) {
      ...
      try {
        return ECKeyFactory
            .getInstance(UnCastleProvider.getInstance())
