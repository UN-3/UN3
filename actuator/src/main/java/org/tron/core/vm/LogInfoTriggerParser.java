...
public class LogInfoTriggerParser {
  ...
  public List<ContractTrigger> parseLogInfos(List<LogInfo> logInfos, Repository deposit) {
    ...
    for (LogInfo logInfo : logInfos) {
      ...
      byte[] contractAddress = TransactionTrace.convertToUnAddress(logInfo.getAddress());
      ...
      String creatorAddr = StringUtil.encode58Check(
          TransactionTrace.convertToUnAddress(contract.getInstance().getOriginAddress().toByteArray()));
