...
import org.un.core.exception.UnException;
...
public class Program {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToUnAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToUnAddress(obtainerAddress.getLast20Bytes());
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = TransactionTrace.convertToUnAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setUnHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(TransactionTrace.convertToUnAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace.convertToUnAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace.convertToUnAddress(getContractAddress().getLast20Bytes());
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(TransactionTrace.convertToUnAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(TransactionTrace.convertToUnAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] unAddr = TransactionTrace.convertToUnAddress(address.getLast20Bytes());
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToUnAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToUnAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(TransactionTrace.convertToUnAddress(getContractAddress().getLast20Bytes()),
            key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToUnAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = TransactionTrace.convertToUnAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToUnAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(UnException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(UnException e) {
      return new PrecompiledContractException(e.getMessage());
    }
