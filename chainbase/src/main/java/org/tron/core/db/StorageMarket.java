...
public class StorageMarket {
  ...
  private long exchange_to_supply(boolean isUN, long quant) {
    logger.info("isUN: " + isUN);
    long balance = isUN ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchange_to_supply2(boolean isUN, long quant) {
    logger.info("isUN: " + isUN);
    long balance = isUN ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isUN, long supplyQuant) {
    long balance = isUN ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isUN) {
    long relay = exchange_to_supply(isUN, from);
    return exchange_from_supply(!isUN, relay);
  }
