...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Un.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Un.proto` file:
...
# or build via protoc
protoc -I=src/main/protos -I=src/main/protos/core --java_out=src/main/java  Un.proto
...
INFO [net] UnNetService start successfully.
